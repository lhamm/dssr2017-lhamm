...
if Strategy<5 || Strategy==7
    ...
    if DorENum==1;
        ...
        Position=5;
    elseif DorENum==2;
        PossErrorTypes  = {'Perfect, CorPatch, CorID', 'Bulls-eye out of frame','Child obscuring bulls-eye', 'CorPatch, WrongID', 'WrongPatch, CorID','WrongPatch, WrongID','No Eye RectWrong', 'No Eye RectCorrect','Eye est, wrong position'};
        Position=6;
    else
...
if Strategy==7 %coding correct images
    FinalTrial                  = GroundTruthStart(Strategy, StrategyList, 1);
    NewCombinationList          = GroundTruthFinalTrialToNCL(FinalTrial, AllCombinations);
    CodeImagesAsYouGo           = 1;
    CorrectPositionStore        = cell((38*8*16*2), 5);
else   
...
elseif Strategy ==5 || Strategy ==6 % This is the case where we are analysing data without coding any of it on the way - we want all combinations
...
%             end
            ...
            if isempty(flist) % Record missing data files
                ...
                if Strategy<5 || Strategy==7 % This is used when we want want user input on each image
                    if VD(i)*100==40
...
%  end
    ...
    elseif Strategy==7
        if exist ('UniqueMissingDataFiles', 'var')
          save(FileNameStrat7, 'CorrectPositionStore', 'UniqueMissingDataFiles');
        else 
           save(FileNameStrat7, 'CorrectPositionStore') 
        end
    end
